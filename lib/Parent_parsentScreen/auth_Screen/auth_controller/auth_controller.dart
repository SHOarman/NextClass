...
//       TextEditingController(); // Holds email
//   final TextEditingController resetOtpController =
//       TextEditingController(); // Holds OTP
...
//
//         // 1. Save Token Logic
//         if (decodedResponse['tokens'] != null &&
//             decodedResponse['tokens']['access'] != null) {
...
//       var headers = {'Content-Type': 'application/json'};
//
//       var body = {"email": createemailController.text, "otp_code": otpCode};
//
...
//         var decodedResponse = jsonDecode(response.body);
//         String errorMessage =
//             decodedResponse['message'] ?? "Verification Failed";
//         Get.snackbar(
//           "Error",
//           errorMessage,
//           backgroundColor: Colors.redAccent,
...
//     try {
//       var headers = {'Content-Type': 'application/json'};
//       var body = {"email": createemailController.text};
//
//       var response = await http.post(
...
//     try {
//       var headers = {'Content-Type': 'application/json'};
//       var body = {"email": forgetPasswordController.text.trim()};
//
...
class AuthController extends GetxController {
  ...
  //====================== Loading State ========================
  // ✅ This variable controls the loader in the UI
  ...
  //         await prefs.setString('token', accessToken);
  //
  //         // ✅ Token print is active now
  //          print("=== Login New Token Saved ===\n$accessToken");
  //       }
  //
  //       // 3. Check User Type and Navigate
  //       String userType = decodedResponse['user']['user_type'];
  //       await prefs.setString('user_type', userType);
  //
  //       if (userType.toLowerCase() == 'tutor') {
  ...
  //     );
  //   } finally {
  //     // 4. Stop Loading (Runs whether success or fail)
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedResponse['tokens'] != null &&
          ...
          // আগের টোকেন ক্লিয়ার করে নতুনটা সেভ করা
          await prefs.remove('token');
          await prefs.setString('token', accessToken);
          // নোট: ChatController এ যদি 'access_token' দিয়ে খুঁজেন, তাহলে এখানেও 'access_token' লিখবেন।
          // বর্তমানে 'token' নামেই সেভ করা হচ্ছে।

          debugPrint("=== Login New Token Saved ===\n$accessToken");
        }

        // ================== ২. USER ID SAVE (এটি মিসিং ছিল!) ==================
        // চ্যাট অপশনে 'My Message' চেনার জন্য এটি বাধ্যতামূলক
        if (decodedResponse['user'] != null &&
            decodedResponse['user']['id'] != null) {
          int userId = decodedResponse['user']['id'];
          await prefs.setInt('user_id', userId);
          debugPrint("=== User ID Saved === $userId");
        } else {
          debugPrint("⚠️ Warning: User ID not found in login response!");
        }
